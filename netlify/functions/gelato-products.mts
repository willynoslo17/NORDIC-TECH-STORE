...
function normalizeEcom(product: any, index: number) {
  const retail = money(product?.price?.basePrice ?? product?.price?.amount ?? product?.price ?? product?.suggestedRetailUsd ?? product?.retailPrice);
  return {
...
export default async (req: Request) => {
  ...
  try {
    ...
    return Response.json({ ok: true, supplier: "gelato", products: [], note: "API key valid. Add ecommerce store products in Gelato (or set GELATO_STORE_ID) for a live priced catalog.", markets: ["NO", "EU", "PE"] });
  } catch (error) {
